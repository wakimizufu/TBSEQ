...
void matrixLED::setRow(int row, uint16_t value) {
    uint16_t _maskbit;
...
uint16_t matrixLED::getRow(int row) {
    uint16_t _ret = 0x00;
    int _rowIndex = row * LED_COL_MAX;
    ...
    for (int i = LED_COL_MAX - 1;1 <= i;i--) {
        _ret = _ret + (uint16_t)_currentLED[_rowIndex + i];
        _ret = _ret << 1;
    }

    //bit0を_retに設定
    _ret = _ret + (uint16_t)_currentLED[_rowIndex];
