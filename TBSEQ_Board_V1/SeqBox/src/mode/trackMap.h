...
*/
	...
	*/
	void getBitstream( int p, unsigned char* _bitstream) {
		...
		if ( ( p < 0) || (p >= TRACKMAP_PATTERN_LENGTH) ){
			p = 0;
		}
		...
		for (int s = 0; s < TRACK_STEP_LENGTH; s++) {
			...
			//演奏パターン (1～8:0x0～0x7)
			_byte = 0x07 & paterns[p].steps[s].pattern;

			//演奏バンク   (0:BANK A ～ 3:BANK D)
			_byte = _byte | tracks[p].trackSteps[s].bank << 3;

			//bit:5 最終ステップ(true:最終ステップ ,false:通常ステップ)
			if (tracks[p].trackSteps[s].lastStep) { _byte = _byte ^ 0x20; }

			*(_bitstream + _bitInd) = _byte;
			_bitInd++;

			//転調( NOTE_PWM_INDEX::NOTE_C2～NOTE_PWM_INDEX::NOTE_C3 を設定)	
			_byte = tracks[p].trackSteps[s].transport - static_cast<unsigned char>(NOTE_PWM_INDEX::NOTE_C2);
			*(_bitstream + _bitInd) = _byte;
			_bitInd++;
		}
	}
